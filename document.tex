...
\vspace{0.5em}
% FOUND BY TESTING 2026-09: this frame's body text used to run past
% the frame boundary and collide with the corner logo -- a single
% \usetheme call can only show the one option combination it was
% actually built with (options aren't per-frame), and the full
% explanation of that plus the nav/fullFooter caveat didn't fit
% alongside the itemize above. Trimmed to one line; both points (and
% the nav/fullFooter collision) are already documented in the README.
\scriptsize Options are set once for the whole document -- see the
README for the full list, defaults, and known collisions
(\texttt{nav}, \texttt{fullFooter}).
\end{frame}
