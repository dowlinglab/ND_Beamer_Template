...
- **A vertically centered multi-line note can overflow past the bottom of the slide** if the corner logo is small/low and the note has more than a line or two — the note's height grows equally above and below its anchor, and there's little room below an anchor that's deliberately close to the page edge. Check the render with your actual content; if it clips, increase `\ndcornerlogomargin`/`\ndcornerlogoheight` (both push the anchor further from the edge) or keep the note shorter.
